...
def fetch_nasa_firms_hotspots(map_key=None, days=1):
    ...
    satellite_events = []
    headers = {
        "User-Agent": "AegisTacticalOSINT/2.4 (NASA FIRMS Hotspot Ingestion)"
    }
    ...
    for iso3, country_name in TARGET_COUNTRIES.items():
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                # Filtrujemy tylko anomalie o wysokiej pewności (high confidence / FRP > 15 MW)
                significant_hotspots = []
                for row in hotspots:
                    confidence = row.get("confidence", "nominal")
                    frp = float(row.get("frp", 0.0) or 0.0) # Fire Radiative Power (moc promieniowania)
                    if confidence in ("h", "high") or frp >= 20.0:
                        significant_hotspots.append(row)
                ...
                if significant_hotspots:
                    ...
                    # Tworzymy zdarzenia dla najbardziej intensywnych ognisk (top 5 na kraj aby nie zaśmiecać)
                    significant_hotspots.sort(key=lambda x: float(x.get("frp", 0) or 0), reverse=True)
                    for spot in significant_hotspots[:5]:
                        lat = float(spot["latitude"])
                        lon = float(spot["longitude"])
                        frp = float(spot.get("frp", 0) or 0)
                        acq_date = spot.get("acq_date", datetime.now(timezone.utc).strftime("%Y-%m-%d"))
                        acq_time = spot.get("acq_time", "1200").zfill(4)
